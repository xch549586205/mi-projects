...
export default class App extends Component {
  ...
  switch = () => {
    console.log(this.SwitchBaseProps, "666");
  ...
  getDeviceProps = (cb) => {
    ...
    Service.spec
      .getPropertiesValue([
        ...
        Object.assign({ did: DeviceID }, this.BrightnessBaseProps),
        Object.assign({ did: DeviceID }, this.TemperatureBaseProps),
      ])
      .then((_) => {
        // console.log(_)
  ...
  formatDeviceProps = (props) => {
    ...
    for (let prop of props) {
      ...
      let siid = prop.siid,
        piid = prop.piid;
      if (
        siid === this.SwitchBaseProps.siid &&
        piid === this.SwitchBaseProps.piid
      ) {
        ret.on = prop.value;
      } else if (
        siid === this.BrightnessBaseProps.siid &&
        piid === this.BrightnessBaseProps.piid
      ) {
        ret.brightness = prop.value;
      } else if (
        siid === this.TemperatureBaseProps.siid &&
        piid === this.TemperatureBaseProps.piid
      ) {
  ...
  updateInstance = (instance) => {
    ...
    let supports = {};
    let supportCount = 0;
    let defs = getDefinitionWithKeyFromInstance(
      instance,
      SwitchKey,
      BrightnessKey,
      TemperatureKey
    ...
    if (brightnessDef) {
      ...
      let [brightnessMin, brightnessMax, brightnessStep] =
        brightnessDef["value-range"];
    ...
    if (temperatureDef) {
      ...
      let [temperatureMin, temperatureMax, temperatureStep] =
        temperatureDef["value-range"];
      supports = Object.assign(supports, {
